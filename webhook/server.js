...
app.post('/webhook', async (req, res) => {
    ...
    const model = new ChatOpenAI({
        openAIApiKey: "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",
...
app.post('/chemistryValue', async (req, res) => {
    ...
    const result1 = await chain.call({
        // input: `solve ${latexVal} mathematically in not more than 5 steps, dont use any word or sentences, only use math numbers and symbols and show me the correct answer`
        // input: `Respond according to the  ${latexVal} in  5 steps and respond appropriately, explain in brief and where its been used.`,
        input: `Analyze the ${latexVal} for chemistry-related problems or questions. The input may include various types of chemistry problems such as stoichiometry, atomic structure, chemical bonding, gas laws, acid-base chemistry, thermodynamics, electrochemistry, organic chemistry, nuclear chemistry, kinetics, analytical chemistry, biochemistry, environmental chemistry, material science, and quantum chemistry.

        Identify the type of chemistry problem or question based on the ${latexVal}.
        
        If the problem involves calculations or equations, solve it step by step and provide a clear explanation of the solution process. Ensure that you cover all relevant types of chemistry problems mentioned.
        
        If the problem is not solvable or requires additional information, inform the user accordingly.
        
        Finally, provide a brief summary of the response, including the type of problem identified, the solution (if applicable), and any additional insights or recommendations.
        
        Ensure accuracy and clarity in your responses, and make sure to address all types of chemistry problems mentioned in the ${latexVal}.`
    });
...
app.post('/chem_user_query', async (req, res) => {
    const prompt = req.body;
    console.log("****************", prompt);
    const value = prompt.inputValue
    const res2 = await chain.call({ input: ` respond according to the ${value} and use the ${value} to prove the point in all of my previous queries ` });
    generations2 = res2.response;
